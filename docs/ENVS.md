...
| `KV_DISK_CACHE_ENABLE` | Opt-in NVMe-backed KV cache (default **0**). `1` adds `--kv-transfer-config` (OffloadingConnector + per-node sharded tier), exports `PYTHONHASHSEED=0`, and keeps `PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True`. Every tier entry below (and `VLLM_ENGINE_READY_TIMEOUT_S` / `VLLM_SKIP_INIT_MEMORY_CHECK`) is supplied by `docker-compose.dspark-disk-tier.override.yml`, which the launcher merges only when this switch is on — an off launch renders the stock service env unchanged. See `kv-disk-tier/`. |
...
| `VLLM_ENGINE_READY_TIMEOUT_S` / `VLLM_SKIP_INIT_MEMORY_CHECK` | Disk-tier delta only, supplied by `docker-compose.dspark-disk-tier.override.yml` (defaults **3600** / **1**): the tier's boot is slower than stock (mount validation + first-touch cache rebuild) and its connector allocates outside vLLM's static check. Neither key is set on a tier-off launch, so the stock registry defaults apply; the Stage-C lane owns `VLLM_SKIP_INIT_MEMORY_CHECK` in `docker-compose.stage-c.override.yml`. |
