...
| `ABLATE` | **Not** `VLLM_*`. Default `0` leaves the image's DeepSeek V4 `model.py` unchanged. `1` stages the configured refusal direction on both nodes and applies the runtime projection hotfix fail-closed. Mutually exclusive with checkpoint selector `ABLITERATED=1`; requires a cold stop/start. |
| `DSV4_ABLATE_LAMBDA` | Runtime projection strength, default `3.5`; parsed only for `ABLATE=1` and must be finite/non-negative. It participates in the AOT-cache compatibility stamp. |
...
| `DSPARK_ABLATE_SOURCE_FILE` | Launcher-side direction source, default `files/direction_r1.pt`. Relative paths resolve from the repo. The launcher stages and SHA-256-verifies it under each node's HF cache; the container sees `/cache/huggingface/dspark-ablation/direction_r1.pt`. |
