...
| `SCHEDULING_POLICY` | vLLM request scheduling policy. Default **`fcfs`** preserves arrival order. `priority` enables the integer `priority` field on supported requests; lower values are admitted before higher values and arrival order breaks ties. It primarily orders waiting requests rather than interrupting active decodes. The entrypoint and preflight validator accept exactly `fcfs`/`priority` and fail before `vllm serve` on any other value. |
