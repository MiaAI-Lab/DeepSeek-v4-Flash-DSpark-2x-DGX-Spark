...
| `DSPARK_LOOP_BREAKER` | **Not** `VLLM_*`. Default `0` (and every non-`1` value) leaves `vllm/v1/engine/detokenizer.py` stock: the patch is not applied and no knob is parsed. Exact `1` applies `patches/hotfix-dsv4-loop-breaker.py` at boot and arms a bounded repeated-line detector that finishes a degenerate generation with `stop_reason=loop-breaker` (issue #82 announce-loop mitigation, not a root-cause fix). Pre-flight (`--check` worker then head) and the boot chain fail closed on a bad knob or drifted source. The launcher resolves the five controls and the selected patcher once on the head and forwards them to worker and worker2 at the canonical `./patches/hotfix-dsv4-loop-breaker.py` path, so a value that exists only in the head environment cannot enable one rank's pre-flight while another stays disabled, and an enabled, unskipped boot refuses a missing or non-regular-file selected patcher before any host is touched. Recreate both ranks after changing it. |
| `DSPARK_LOOP_BREAKER_REPEATS` / `DSPARK_LOOP_BREAKER_SHORT_REPEATS` / `DSPARK_LOOP_BREAKER_MIN_TOKENS` | Read and range-checked **only** while `DSPARK_LOOP_BREAKER=1`: prose-line repeats (`2`-`1024`, default `6`), short sentence-like-line repeats (`2`-`1024`, default `15`), output-token floor (`0`-`1000000`, default `64`). Repeats must land inside a bounded counting window; fenced/indented code, JSON/tool-call lines and DSML blocks are never counted. A malformed or out-of-range value aborts the pre-flight and the boot chain (exit 1, variable named) instead of defaulting silently. |
| `DSPARK_SKIP_LOOP_BREAKER_HOTFIX` | `1` skips applying `patches/hotfix-dsv4-loop-breaker.py` (only meaningful with `DSPARK_LOOP_BREAKER=1`; a `--status` query still inspects the file) |
