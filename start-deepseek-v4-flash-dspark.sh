...
# Optional runtime refusal-direction ablation (distinct from the abliterated
# checkpoint selector above). Keep the public knobs compatible with the 1x
# Spark recipe, but stage the host file into both nodes' existing HF mounts.
ABLATE="${ABLATE:-0}"
...
if [ "$ABLATE" = "1" ] && [ "${ABLITERATED:-0}" = "1" ]; then
  echo "ABLATE=1 and ABLITERATED=1 cannot be combined; use official weights for runtime ablation or disable ABLATE." >&2
  exit 2
fi
if [ "$ABLATE" = "1" ]; then
