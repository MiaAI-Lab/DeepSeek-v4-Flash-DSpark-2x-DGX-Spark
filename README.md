...
| `DSPARK_MAX_INFLIGHT_PREFILLS` | `1` | Issue **#27** in-flight partial prefills (1–3). Default `1` (strictly serialized): the post-#211 exact admission gate is live-qualified at 1 (decode-fairness spread 1.60–1.76×, zero preemptions, repeated fresh boots). `2`–`3` are evidence-backed opt-ins: `2` is operator-qualified post-r3 on TP=2 with `LONG_PREFILL_TOKEN_THRESHOLD=1024` ([#217](https://github.com/MiaAI-Lab/DeepSeek-v4-Flash-DSpark-2x-DGX-Spark/issues/217): 4 × 8K gate26 spread 1.50–1.57×, zero preemptions, TTFT spread 2.1–2.4× vs 4.1× at `1`), trading admission serialization for TTFT/equity on admission-limited shapes. The 2026-09-02 A/B that first favored `2` predates the r3 counting fix. |
