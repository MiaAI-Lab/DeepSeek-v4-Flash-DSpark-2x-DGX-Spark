...
- **Upstream vLLM #51967 backport wired and gated: compile-time constants for the global top-k index kernel** (`patches/hotfix-dsv4-topk-compile-time-consts.py`): promotes `global_topk_indices_stride`, `topk_indices_stride`, `topk`, `block_table_stride`, and `block_size` in `_compute_global_topk_indices_and_lens_kernel` from runtime arguments to `tl.constexpr` — byte-for-byte the five promotions upstream merged on 2026-08-16. The patcher itself is unchanged in substance (same anchor, same single `replace(..., 1)`, same idempotence marker, same `SystemExit` on anchor miss); what is new is that it is now reachable. It gained a `--status` read-only probe and honours `VLLM_ROOT` (the convention PR #73 settled), `docker-compose.dspark.yml` mounts it at `/opt/` and invokes it behind an exactly-`1` `DSPARK_ENABLE_TOPK_CONSTEXPR_HOTFIX` gate with `|| exit 1`, and `start-deepseek-v4-flash-dspark.sh` scps it to the worker so rank 1 actually has the file. Because each rank's container runs the entrypoint, one gate covers both ranks. Default `0` = stock kernel, so `<=0.80` serving behaviour is unchanged unless the gate is set.

  **Evidence status, stated exactly: unmeasured on this fork.** The previous docstring's "decode C1 +3.3% (47.8 vs 46.3 tok/s)" is **withdrawn** — it carried no n, no dispersion and no test; its only statistics (`Welch p=0.002, n=16+16`) belonged to a three-patch `#51967+#49283+#49160` stack and cannot be attributed to this patch; and no artifact under `results/` reproduces it. The docstring's mechanism claims are corrected too: this kernel is called on the C4A decode path (whose persistent padded `topk_indices` buffer and `is_valid_token` mask are the reason it exists) as well as prefill, not "every prefill step"; the reachable constants on this fork are `topk` = 512 with `block_size` = 64 on C4A and `block_size` = 2 on C128A prefill, not `topk=128 / block_size=256`; and the promotion does **not** unroll the loop, because `for i in range(0, topk, TRITON_BLOCK_SIZE)` already receives `TRITON_BLOCK_SIZE=1024` as a `tl.constexpr`, making the C4A loop a single trip. Upstream's own measurement for this change is +0.50% mean output throughput / −0.98% mean TPOT at TP=8. The fork-local effect stays unknown until a baseline → candidate → baseline run records it.

  CPU gates: `scripts/test-topk-constexpr-patchapply.py` applies the patcher to a copy of the vendored `recipe/overlay/.../cache_utils.py` via `VLLM_ROOT` and asserts the anchor occurs exactly once, the resulting diff is exactly upstream #51967's 5-added/5-removed hunk, a second run is a byte-identical `[skip]`, `--status` reads `NOT APPLIED` then `APPLIED` without writing, and a corrupted anchor exits non-zero leaving the file byte-identical. `scripts/ci-validate.sh` additionally asserts the compose gate and the worker sync so the wiring cannot silently regress.
