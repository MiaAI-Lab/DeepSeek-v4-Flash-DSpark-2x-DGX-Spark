...
- **Optional KV/execution profile switches (`KV_CACHE_DTYPE`, `ENABLE_DSPARK_SPECULATIVE`, `ENFORCE_EAGER`; PR #63)**: three opt-in knobs, validated fail-closed in `start-deepseek-v4-flash-dspark.sh` *before* export (`exit 2` outside the `nvfp4_ds_mla|fp8|fp8_ds_mla` whitelist or outside `0|1`), and passed to every compose invocation that renders the serve argv on both ranks (head `compose_base`, worker `config --quiet`, worker `up -d`). Defaults are unchanged — `nvfp4_ds_mla`, speculation on, CUDA graphs on — and the rendered `vllm serve` argv at defaults is word-for-word identical to the previous default; `scripts/test-kv-profile-switches.py` pins that 61-word baseline argv, the 3×2×2 render matrix, the rejection matrix, both worker propagation sites, and the resolved-profile output.

  **No memory or precision trade-off exists between the dtypes**, contrary to the rationale this PR originally shipped. Both `nvfp4_ds_mla` and `fp8_ds_mla` use the same 584-byte-per-token DSV4 sparse-MLA record (448 B fp8 NoPE + 128 B bf16 RoPE + 8 B ue8m0 scales — `recipe/overlay/vllm/models/deepseek_v4/common/ops/cache_utils.py`), which is precisely why the Issue #22 hotfix can route both dtypes through one kernel that strides that record. The KV token pool is therefore unchanged at equal `GPU_MEMORY_UTILIZATION`. The two datapoints quoted for this switch reduce to **7,787 B/token** (18.08 GiB / 2,493,464 tok @ util 0.835 — `results/RESULTS-2026-08-14.md`) versus **7,762 B/token** (12.94 GiB / ~1.79M tok @ util 0.80 — operator-reported, *not* in `results/`): 0.3% apart, so the pool difference is the utilization difference, not bit width. The "~2x the latent bytes → smaller token pool" and "higher precision / better long-context recall" claims were therefore removed from `.env.dspark.example` and from the start script's resolved-profile output. Post-#22 both dtypes dispatch to the same `_forward_fp8_kv` kernel; no recall difference has been measured (`scripts/ruler-lite.py` is the instrument if anyone wants to).

  `fp8` stays an accepted spelling, but `fp8_ds_mla` is documented as the preferred one: the Issue #22 hotfix predicate names only `fp8_ds_mla` and `nvfp4_ds_mla`, so a bare `fp8` depends on the image normalizing it before that predicate runs — not verifiable from this repo, because the DSV4 MLA front door and `flashmla_sparse.py` exist only inside the pinned image. Unverified, so it is documented as an assumption rather than asserted as fact.
### Changed
...
- **`--max-cudagraph-capture-size` stayed MTP-derived with speculation off or eager execution on (PR #63)**: the flag was unconditionally `MAX_NUM_SEQS * (MTP_NUM_TOKENS + 1)` = 36, while the decode query length upstream sizes graphs against is `1 + num_speculative_tokens`. With `ENABLE_DSPARK_SPECULATIVE=0` that captured 6× more batch sizes than can ever occur, and the start script printed `mtp speculative tokens: inactive` directly above `cudagraph capture size: 36`. The compose entrypoint now derives `DECODE_QUERY_LEN` inside the existing speculation gate (`MTP_NUM_TOKENS + 1` on, `1` off) and drops the flag entirely under `ENFORCE_EAGER=1`, where no graphs are captured and a capture size is inert. `start-deepseek-v4-flash-dspark.sh` and `validate-dspark-config.sh` print the same three cases instead of one unconditional number. At defaults the argv is unchanged: `--max-cudagraph-capture-size 36`, same position.
