...
- **GB10 vLLM IPC spin-wait (CPU load / SoC heat) — new hotfix** (`patches/hotfix-gb10-spin-wait.sh`): vLLM's internal IPC receive path (`vllm/distributed/device_communicators/shm_broadcast.py`, `SpinCondition`) spins on performance cores for `busy_loop_s` (default **1s**) before sleeping on its notify socket. Decode messages always arrive < 1s apart, so the sleep path never engages and 3–4 P-cores spin indefinitely at max clock. On a single-die GB10 this wasted spin is real heat (reported CPU 333%→89%, SoC −11°C, no throughput/latency change — nacyot, "vllm-spin-wait-gb10"). Applies to **TP≥2 only** (this 2-node recipe); single-GPU/`world_size=1` uses the uni executor and never forms this path. The hotfix flips the default to `busy_loop_s: float = 0.002` (2ms grace then real sleep), is idempotent and self-verifying, and is synced to both workers and applied in the compose entrypoint before `exec vllm` — no image rebuild. Opt out with `DSPARK_SKIP_SPIN_WAIT_HOTFIX=1`. Also wired into `start-…sh` sync, `docs/PATCHES.md` (root cause + scope), and `docs/ENVS.md`.
