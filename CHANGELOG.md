...
- **GB10 shm spin-wait pins ~4 P-cores during decode → thermal risk** (`patches/hotfix-vllm-shm-spinwait-gb10.py`): on GB10 (DGX Spark / GX10) with TP≥2, vLLM's shm ring-buffer `SpinCondition` busy-loops via `sched_yield()` for `busy_loop_s=1s` after each read before falling back to sleep. During decode a new IPC message always lands inside that 1s window, so the sleep path is never taken — ~4 Grace P-cores stay pinned at full clock doing no work (~333% CPU; measured cores 6/9/17/19 at ~90–100%, `VLLM::Worker` ~300% + `EngineCore` ~100%) while the GPU sips ~38W at ~96% util. That drives the CPU cluster to ~84°C, roughly half of GB10's shared thermal budget (hard-shutdown at 104.8°C) — a likely contributor to thermal blackouts. The hotfix lowers `busy_loop_s` 1s → 2ms at boot (idempotent; refuses to patch if the upstream default changed): CPU ~333% → ~89%, SoC ~−11°C, decode throughput unchanged (GPU stays ~96%). Single-unit TP=1 boxes use the in-process executor and are unaffected. Skip with `DSPARK_SKIP_SPINWAIT_HOTFIX=1`. Analysis and measurements: <https://nacyot.github.io/artifacts/vllm-spin-wait-gb10-en/>.
