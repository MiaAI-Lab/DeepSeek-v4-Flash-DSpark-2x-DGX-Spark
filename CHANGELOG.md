...
- **Dual-HCA `NCCL_IB_GID_INDEX` pin no longer survives into the container (tonyd2wild #38)**: `start-*.sh` still resolves and logs a common RoCEv2 GID index from sysfs (`NCCL_IB_GID_AUTO=1`), but by default it now **leaves `NCCL_IB_GID_INDEX` unset** so NCCL picks the RoCEv2/IPv4 GID per HCA. A pinned index is one global value per rank; on dual-HCA the table drifts after link/fw events and a stale pin of `3` hangs with `ibv_modify_qp 61`. Empty-string injection is treated as GID 0 (fe80), so `remote_nccl_env` omits the key when empty and the compose entrypoint `unset`s it if compose still interpolates a blank. Restore the old pin-the-resolved-index behaviour with `NCCL_IB_GID_LEAVE_UNSET=0`; explicit `NCCL_IB_GID_AUTO=0` pins still win. Field-tested on 2× GB10, Anemll `dspark-vllm-gx10:0.1.1`, TP=2, both HCAs up: boot ~6 min, vLLM process has `NCCL_IB_HCA` and no `NCCL_IB_GID_INDEX`, decode flat vs the pinned-3 baseline.
