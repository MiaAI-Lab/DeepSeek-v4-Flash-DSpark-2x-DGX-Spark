...
- **Upstream vLLM #49283 backport: per-task Q/KV RMSNorm block widths, wired default OFF (`DSPARK_ENABLE_QK_RMSNORM_SPLIT`, default `0` = stock kernel)** (`patches/hotfix-dsv4-qk-rmsnorm-split-blocks.py`): the fused Q/KV RMSNorm kernel launched both grid-y tasks with one shared `BLOCK_SIZE = next_power_of_2(max(q_size, kv_size))`. The backport extracts a reusable `_rmsnorm_row` helper and gives each task its own compile-time width. The patcher is now mounted, scp'd to the worker by `start-deepseek-v4-flash-dspark.sh`, and invoked by the compose entrypoint on each rank only when the flag is exactly `1`, chained with `|| exit 1`. It gained `--status` (applied / not-applied / drifted + target sha256 + image vLLM version, exit 0/1/2), `--dry-run` (predicts anchors *and* the resulting self-check, exit 0/2), exact-occurrence anchor assertions, a post-write structural self-check that **restores the original bytes** on failure, and re-validation instead of a bare skip on an already-patched target. Neither `--status` nor `--dry-run` ever writes.

  **Corrected rationale.** The previous docstring copied upstream's premise ("on DeepSeek-V4 the Q row is narrower than the KV row"). It is inverted for the deployed checkpoint: `DeepSeek-V4-Flash-0731` sets `q_lora_rank=1024` and `head_dim=512`, so `Q_BLOCK` stays 1024 (and 1024 is already a power of two, so the Q row never had a masked lane and is bit-identical), and the entire effect is `KV_BLOCK` 1024 → 512. Upstream's 1.19–1.34× kernel microbenchmark is for `q=192/kv=576` on an Intel B60 and does not transfer. Same failure mode #48407 already documents in `docs/vllm-027-new-patches.md`.

  **Evidence status, stated exactly: no end-to-end measurement is claimed.** Verified on CPU and re-asserted every CI run by `scripts/test-qk-rmsnorm-split-blocks.py`: applying the patcher to upstream's frozen pre-image `8688a06` (sha256 `8ea5fd82ab09db66…`) reproduces upstream's frozen post-image `419d610a` (sha256 `cb5262282376c5c4…`) **byte for byte**; five anchors, each exactly once; idempotent and byte-stable on re-run; anchor drift refused with nothing written; self-check failure restores the original bytes; `--status`/`--dry-run` never write. **Removed, not restated:** the earlier `47.8 → 49.7 tok/s (+4.0%)` and stacked `46.3 → 52.0 (+12.2%, Welch p=0.002)` decode claims. They were unattributable to this patch (the launch count is unchanged and only one of two tasks narrows on a weight-bandwidth-bound decode), their stack baselines are not in `main`, and the "re-tested 2026-08-17" label sits on a commit dated 2026-08-16. **Also not verified:** that the anchors match the deployed image — they are proven against upstream `8688a06` (2026-07-21) while the pinned image tree is `g752a3a504` (2026-07-14); `--dry-run` inside the container closes that gap without mutating it. Numerically the Q row is bitwise unchanged and the KV row is bounded at ~1 ulp fp32 in `variance` (block width can change the fp32 summation order), so bitwise A/B gates on the KV row are invalid, and the patch must never be applied to one rank only — the latent is TP-replicated (`disable_tp=True`).
### Changed
