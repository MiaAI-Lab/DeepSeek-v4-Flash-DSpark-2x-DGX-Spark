...
- **Live multi-turn tool-history verifier for [Issue #82](https://github.com/MiaAI-Lab/DeepSeek-v4-Flash-DSpark-2x-DGX-Spark/issues/82)**: `scripts/reproduce-issue82-live.py` grows an actual OpenAI-format conversation through alternating required-tool and required-no-tool turns instead of replaying one frozen request. It can seed a 41-message synthetic history approximation, replay or drop prior assistant reasoning as an A/B, add unique synthetic long-context evidence, and retain immutable, hash-verifiable request snapshots plus raw pre-parser output reconstructed from returned output token IDs. A one-time `/tokenize` → `/detokenize` proof verifies that model-produced reasoning is present in the server-rendered replay prompt and absent from the drop prompt. Replay accepts both `reasoning` and the deprecated `reasoning_content` response alias and exits nonzero unless model-produced reasoning reaches a subsequent request. Optional `--thinking-token-budget`, `--temperature`, and `--top-p` flags cover the captured budget arm and sampled-trajectory canaries while preserving the deterministic no-budget defaults. Malformed responses and detokenization failures remain in partial failing artifacts instead of aborting the run. The gate rejects stale or invalid calls, `count=<n>`, false `<system>` tags, length exhaustion, parser/DSML mismatches, and run-wide reasoning or call repetition at 4 occurrences (the public healthy control peaked at 3; the pathological control reached 16). The paired CPU suite and `scripts/EVAL.md` document deterministic use. Diagnostic only: current `main` did not reproduce the attractor with this synthetic approximation, the reporter's private 46-message final-user fixture and free-running 20-turn driver remain unavailable, and no tokenizer/parser/model behavior changes.
